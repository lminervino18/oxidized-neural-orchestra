    ...
    W: AsyncWrite + Unpin + Send,
{
    ...
    let mut server_handle = ParamServerHandle::new(server_id, transport);
    ...
    W: AsyncWrite + Unpin + Send,
{
    ...
    let mut worker_handle = WorkerHandle::new(worker_id, transport);
...
async fn test_lineal_convergence() -> io::Result<()> {
    ...
    let transport = comms::build_simple_transport(sv_rx, sv_tx);
    let worker_handle = WorkerHandle::new(worker_id, transport);
    server.spawn(worker_handle);

    let worker_fut = mock_lineal_worker(
        wk_rx, wk_tx, wk_orch_rx, wk_orch_tx, MAX_EPOCHS, NPARAMS, orch_id, server_id,
    );
    let orchestrator_fut = mock_orch(
        orch_wk_rx, orch_wk_tx, orch_sv_rx, orch_sv_tx, worker_id, server_id,
    );
