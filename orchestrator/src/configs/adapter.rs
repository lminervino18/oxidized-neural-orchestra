...
use comms::specs::{
    ...
    worker::{SerializerSpec, WorkerSpec},
};
...
impl Adapter {
    ...
    pub fn adapt_configs<'a>(
        ...
        match &training.algorithm {
            ...
            AlgorithmConfig::RingAllReduce => {
                let workers =
                    self.adapt_ring_all_reduce_workers(&model, training, dataset_specs)?;
    ...
    fn adapt_parameter_server_workers(
        ...
        let worker_specs = training
            ...
            .zip(dataset_specs)
            .map(|((i, addr), dataset)| {
                self.validate_socket_addr(
                    addr,
                    format!("failed to resolve {i}'th worker's network address: {addr}"),
                )?;
    ...
    fn adapt_servers(
        ...
        let (servers, server_sizes): (Vec<_>, Vec<_>) = server_addrs
            ...
            .map(|(i, (addr, (param_gen_spec, size)))| {
                self.validate_socket_addr(
                    addr,
                    format!("failed to resolve {i}'th server's network address: {addr}"),
                )?;
