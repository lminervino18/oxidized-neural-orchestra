...
sistema de entrenamiento distribuido propio en el que todos los nodos son id\'enticos y
un orquestador les asigna el rol (worker o servidor) en tiempo de ejecuci\'on. Sobre
O.N.O.\ comparamos AR y PS (este \'ultimo sincr\'onico, con barrera) en MNIST, bajo un
criterio de \emph{batch global fijo} (necesario porque en O.N.O.\ el \texttt{batch\_size}
es por worker), midiendo convergencia, throughput y escalabilidad. La evidencia observada
indica que, en este entorno homog\'eneo sobre una sola m\'aquina, All-Reduce iguala a
Parameter Server en convergencia y lo supera en throughput y tiempo hasta accuracy, porque
el servidor central de PS introduce un cuello de serializaci\'on. PS, en cambio, escala con
pendiente m\'as pronunciada al crecer los nodos; sus dem\'as ventajas estructurales
(sharding, heterogeneidad, asincron\'ia) no se ejercitan en este entorno.
Las conclusiones est\'an acotadas a un cl\'uster simulado sobre una \'unica m\'aquina de
ocho n\'ucleos.
\end{abstract}

\begin{IEEEkeywords}
entrenamiento distribuido, deep learning, Parameter Server, All-Reduce, agregaci\'on de
gradientes, escalabilidad.
\end{IEEEkeywords}
...
\textbf{Contribuci\'on.} (i) Describimos O.N.O., un sistema distribuido propio donde los
roles se asignan en runtime y que implementa AR y PS; (ii) aportamos una comparaci\'on
experimental honesta AR vs.\ PS sobre MNIST con un criterio de comparaci\'on justa
(\emph{batch global fijo}); y (iii) destilamos una matriz de decisi\'on sobre cu\'ando
preferir cada enfoque, acotada a las condiciones evaluadas.
...
En O.N.O.\ todos los nodos son id\'enticos y agn\'osticos al rol. Un
\texttt{orquestador} headless se conecta a ellos y, mediante una configuraci\'on de
runtime, fija el rol de cada uno (worker o servidor): los roles \emph{no} est\'an
fijados de antemano, sino que se asignan en cada ejecuci\'on. El sistema separa el motor
de aprendizaje, la comunicaci\'on (protocolo y serializaci\'on) y las estrategias de
agregaci\'on en componentes independientes, y est\'a implementado en Rust.
...
Comparamos dos estrategias distribuidas: All-Reduce (todos los nodos son workers) y
Parameter Server (barrera con store de doble buffer). Las topolog\'ias distribuidas se
detallan en \cref{tab:configs}. Para PS con $N$ nodos usamos $N-1$ workers y $1$ servidor.
Evaluamos $N\in\{3,5\}$.
...
\textbf{(1) Batch global fijo}: fijamos $W\cdot b$ y derivamos $b$ seg\'un el n\'umero de
workers, lo que mantiene la sem\'antica de optimizaci\'on constante y es el criterio
v\'alido para \emph{convergencia}. \textbf{(2) Batch por worker fijo}: $b$ constante, que
mide \emph{throughput bruto} pero cambia el batch efectivo y por ende no es justo para
convergencia. Reportamos ambos y somos expl\'icitos sobre qu\'e pregunta responde cada uno.
...
Ambas estrategias entrenan correctamente: con la receta de referencia alcanzan en MNIST
una accuracy de test cercana al \'optimo del dataset, con PS convergiendo marginalmente
m\'as bajo en p\'erdida. \Cref{fig:loss} muestra que las curvas de p\'erdida distribuidas
descienden de forma casi solapada, lo que confirma que ambos esquemas de agregaci\'on
implementan la misma sem\'antica de optimizaci\'on.
...
Bajo \emph{batch global fijo}, AR y PS convergen a una accuracy pr\'acticamente igual
(\cref{tab:main}); el batch global mayor reduce la accuracy respecto de la receta de
referencia, como era de esperar. La diferencia decisiva es el \emph{tiempo}: AR alcanza
su accuracy objetivo apreciablemente antes que PS (\cref{tab:main}), porque el servidor
central de PS act\'ua como punto de serializaci\'on por el que pasan todos los updates.

\input{tables/main_results.tex}

\subsection{Throughput y escalabilidad}
A batch por worker fijo, AR sostiene mayor throughput que PS en ambos tama\~nos
(\cref{fig:throughput}). Sin embargo, PS \emph{escala} con pendiente m\'as pronunciada
(\cref{fig:speedup}): al sumar nodos su throughput crece m\'as r\'apido que el de AR, de
modo que casi se igualan a 5 nodos. La lectura: el servidor de PS penaliza el caso chico
pero su costo fijo se amortiza al sumar workers; AR parte m\'as alto pero su anillo
...
Para aislar el efecto del tama\~no de gradiente entrenamos MLPs de tama\~no creciente
(\cref{tab:models}) a 3 nodos, con batch por worker fijo. \Cref{fig:comm} (eje
logar\'itmico) muestra que el throughput se desploma al crecer el modelo: la
comunicaci\'on, no el c\'omputo, domina. Comparando estrategias, All-Reduce sostiene
mayor throughput que Parameter Server tanto con el modelo chico como con el grande, y
empatan en el tama\~no intermedio. La presi\'on de comunicaci\'on hunde el throughput
absoluto pero no invierte la ventaja de All-Reduce: su anillo, sin servidor central,
sigue siendo m\'as r\'apido aun cuando el gradiente denso domina el costo. La
compresi\'on de gradientes, esparsa o cuantizada, es una mitigaci\'on conocida para esta
...
\begin{figure}[t]
  ...
  nodos. All-Reduce sostiene mayor throughput que Parameter Server en los extremos
  (modelo chico y grande) y empatan en el intermedio; el throughput absoluto se
  desploma al crecer el gradiente.}
  \label{fig:comm}
\end{figure}
...
\textbf{Cu\'ando conviene Parameter Server.} La evidencia favorece a PS en un \'unico
aspecto del entorno evaluado: escala con mayor pendiente al sumar nodos, amortizando su
costo fijo de servidor. Su ventaja te\'orica m\'as fuerte ---el sharding de
par\'ametros--- ser\'ia decisiva para modelos que no entran en un solo nodo, pero el
mayor que evaluamos s\'i complet\'o en All-Reduce, as\'i que no se ejercit\'o
(\cref{fig:comm}). Lo mismo con sus dem\'as ventajas (heterogeneidad, stragglers,
asincron\'ia), que quedan como trabajo futuro.
...
PS recorta distancia al escalar (mayor pendiente desde una base m\'as baja), pero no
super\'o a All-Reduce en throughput ni siquiera con el modelo m\'as grande; ser\'ia la
elecci\'on natural solo si se explotaran sus ventajas estructurales (heterogeneidad,
asincron\'ia, sharding de modelos que no entran en un nodo), que aqu\'i no se ejercitaron. O.N.O.\ aporta evidencia de que un sistema distribuido
