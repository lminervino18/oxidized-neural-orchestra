...
fn main() {
    ...
    let epochs = NonZeroUsize::new(10).unwrap();
    ...
    let train_dataset = make_dummy_dataset(10 * batch_size.get(), &mut rng);
...
fn make_dummy_dataset(size: usize, rng: &mut impl Rng) -> Dataset {
    let samples = vec![rng.random_range(0.0..1.0); size * SAMPLE_SIZE * size_of::<f32>()];
    let labels = vec![rng.random_range(0.0..1.0); size * LABEL_SIZE * size_of::<f32>()];
