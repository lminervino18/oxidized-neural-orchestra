...
pub trait LossFn {
    /// Returns the loss of the predicted output `y` with respect to the expected one `y`.
    ///
    /// # Arguments
    /// * `y_pred` - The computed output
    /// * `y` - The expected *real* output
    fn loss(&self, y_pred: ArrayView2<f32>, y: ArrayView2<f32>) -> f32;
    /// Returns the derivative of the loss of the predicted output `y` with respect to the expected one `y`.
    ///
    /// # Arguments
    /// * `y_pred` - The computed output
    /// * `y` - The expected *real* output
